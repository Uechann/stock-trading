...
- [x] 종목이 빈칸 이거나 비어있을 경우 예외 처리
- [x] 종목 형식 -> 알파벳 대문자 1~5자리 형식 맞지 않을 경우 예외 처리
- [x] 중복된 종목 입력 시 예외 처리
  테스트 케이스
- [x] `` -> 실패
- [x] ` ` -> 실패
- [x] `AAPL,GOOG,MSFT` -> 성공
- [x] `AAPL,GOOG,AAPL` -> 실패
- [x] `AAPL,GOOG,MSFT123` -> 실패
- [x] `AAPL,GOOG,msft` -> 실패

---

#### 2. 계좌 번호 생성 및 자금, 종목 목록 보유량 초기화 기능
계좌 클래스 이름: account
계좌 자금 이름 : funds
목록 보유량 : Quantity
...
- 초기 자금
    ...
    - [x] `abc123` -> 실패
- 주식 보유 수량
    - [x] `APPL=50,GOOG=30` -> 성공
    - [x] `APPL=abc,GOOG=30` -> 실패
    - [x] `APPL=-10,GOOG=30` -> 실패
    - [x] `APPL50,GOOG=30` -> 실패
    - [x] 수량 최대 한도 (10,000) 초과 시 예외 처리
    - [x] `` -> 성공 (보유 수량 0)
    - [x] ` ` -> 성공 (보유 수량 0)

---

#### 3. 다음 단계 이동 기능 구현 및 초기화 기능 통합

사용자가 초기화 단계를 마치고 주문 입력 단계로 넘어갈 수 있도록 구현합니다.

세부 구현

- 계좌 생성, 자금 초기화, 종목 보유량 초기화 기능 통합
- `NEXT` 명령어 입력 시 다음 단계로 이동
- 초기화가 완료되지 않은 상태에서 `NEXT` 명령어 입력 시 예외 처리

예외 처리
- [x] 초기화가 완료되지 않은 상태에서 `NEXT` 명령어 입력 시 예외 처리
- [x] `NEXT` 명령어 외의 잘못된 명령어 입력 시 예외 처리

---

#### 3-2. 기능 통합 및 DI 컨테이너 구현 및 저장소 구현
Controll, service, 등 의존성 DI 컨테이너 구현합니다.
저장소 repository 인터페이스를 구현하여 service에서 해당 데이터가 유지될 수 있도록 구현합니다.
...
- 계좌
    - [x] 계좌가 공백이거나 비어있을 경우 예외 처리
    - [x] 존재하지 않는 계좌 입력 시 예외 처리
    - [x] 계좌 형식이 올바르지 않을 경우 예외 처리
- 종목
    - [x] 종목이 빈칸 이거나 비어있을 경우 예외 처리
    - [x] 종목 형식 -> 알파벳 대문자 1~5자리 형식 맞지 않을 경우 예외 처리
    - [x] 존재하지 않는 종목 입력 시 예외 처리
- 매도/매수
    - [x] 빈칸이거나 비어있을 경우 예외 처리
    - [x] 매수 : BUY, 매도 : SELL 외의 값 입력 시 예외 처리
- 가격
    - [x] 가격이 빈칸 이거나 비어있을 경우 예외 처리
    - [x] 가격이 숫자가 아닐 경우 예외 처리
    - [x] 가격이 음수일 경우 예외 처리
    - [x] 가격이 0일 경우 예외 처리
    - [x] 가격 최대 한도 (1,000,000) 초과 시 예외 처리
    - [x] 가격 소수점 입력 시 예외 처리
- 수량
    - [x] 수량이 빈칸 이거나 비어있을 경우 예외 처리
    - [x] 수량이 숫자가 아닐 경우 예외 처리
    - [x] 수량이 음수일 경우 예외 처리
    - [x] 수량이 0일 경우 예외 처리
    - [x] 수량 최대 한도 (10,000) 초과 시 예외 처리
- 가격 X 수량 
    - [x] 가격 X 수량이 3,000,000 초과 시 예외 처리
    - [x] 매수 주문 시 계좌의 현금이 부족할 경우 예외 처리
    - [x] 매도 주문 시 계좌의 보유 수량이 부족할 경우 예외 처리
...
매칭 엔진 흐름
1. 저장된 Order를 통해 각 종목별 OrderBook 생성
2. OrderBook에는 해당 종목과 매수 큐, 매도 큐가 존재한다.
3. 가격 교차에 대한 검증은 매수 큐는 최대 가격 우선 순위, 매도 큐는 최소 가격 우선 순위로 결정한다.
4. 주문이 생성되면 각 종목별 OrderBook에 하나씩 add 한다.
5. 매칭 엔진을 실행하여 종목의 한 사이클 동안 매수 큐, 매도 큐에 주문이 존재하는지 검증한다.
6. 하나씩 꺼내어 교차 가격 (매수 가격 > 매도 가격)이 아니면 종료한다.
7. 이때 구매할 가격은 일반적인 규칙(매도 가격)으로 결정한다.
8. 이때 구매할 수량은 매도할의 수량으로 결정한다.
9. Trade에 해당하는 객체를 생성한다. (매수자, 매도자, 종목, 가격, 수량, 시간 등등)
10. 각 주문에 대한 남은 수량이 있다면 업데이트를 한다. 
11. 모든 수량이 끝났다면 Order는 Complete로 상태를 변경하고 큐에서도 dequeue 한다.

예외 상황

- 매칭 불가 상황
    - [x] 매수 가격 < 매도 가격 일 경우 매칭 불가
    - [x] 매도 주문이 없을 경우 매칭 불가
    - [x] 매수 주문이 없을 경우 매칭 불가
    - [x] 동일 종목이 아닐 경우 매칭 불가
...
예외 상황
- 음수 금지 정책
    - [x] 매수자 현금이 음수가 될 경우 예외 처리
    - [x] 매도자 보유수량이 음수가 될 경우 예외 처리
...
예외 상황
- [x] 존재하지 않는 ORDER_ID 입력 시 예외 처리
- [x] 이미 체결된 주문 취소 시 예외 처리
- [x] OrderBook에 값이 없는 경우 예외 처리
...
- 체결 결과 출력
    ...
    - TradeRepository 저장을 통해 각 체결 결과들 출력
      - ID가 필요할까 ?
      - 일급 컬렉션을 도입할까 ?
      - List로 관리할까 Map으로 관리할까 ?
    - Trade의 용도를 먼저 생각
      - 체결 이벤트 로그에 가깝기 때문에 이미 끝난 사건이고 다시 수정이나 삭제를 이루어지지 않는다.
      - ID는 현재 필요하지 않다
      - 일급 컬렉션으로 조회를 할때 도메인 편의를 돕자
      - id가 없는 상황에서는 List로 충분히 조회가 가능
...
- 각 계좌 별 잔액 및 포지션 요약 평가 손익, 수익률 출력
    ...
    - ```
      ...
      ```
    - 수정 사항: 
    - SymbolPriceProvider 인터페이스 도입 (외부 시세 데이터 조회 확장성 고려) 
      - 상속 클래스 InMemorySymbolPrice을 통해 각 종목당 현재가(시세) 저장 (최종 체결가로 처리)
      - 종목 초기화 할때 초기가 10,000원으로 설정
      - 계좌에서 종목 보유량 설정 시에 매입가(10,000원으로 가정) 반영해서 총 매입가 데이터 저장 
      - 매칭이 일어날 때마다 종목당 체결가로 시세 업데이트 처리
    - AccountSymbol -> Position으로 변경 (총 매입가 데이터 추가)
      - 현재 각 Account -> AccountSymbols(종목, 수량) 관리 되던 것에 + (총 매입가) 데이터 추가
      - 수정 사항: 정산이 이루어질때 체결 결과를 업데이트
      - 매수가 이루어졌으면 기존 총 매입가 + (체결 가격 * 수량) 계산해서 반영
      - 매도가 이루어졌으면 (기존 총 매입가 * 남은 수량 / 기존 수량) 계산해서 반영 (평균 원가 방식으로 계산)

    - 평가 손익 수익률은 SummaryService에서 계산해서 출력

- 종목별 통계 (체결 건수, 평균 체결가, 최종 체결가) 출력
    - <종목>: 체결 <체결건수>건, 평균체결가 <평균체결가>, 최종체결가 <최종체결가>
    - `AAPL: 체결 10건, 평균체결가 118, 최종체결가 120`
    - TradRepository에서 체결들을 순회 하면서 체결 건수와 평균 체결가 계산
    - 최종 체결가는 MarketPriceBoard에서 조회하여 반영
